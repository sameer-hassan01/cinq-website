        ...
        {work.map((p) => {
          ...
          return (
            <article
              key={p.slug}
              className={cn(
                "relative flex shrink-0 flex-col overflow-hidden rounded-card lg:w-[min(84vw,1240px)] lg:flex-row",
                tones[p.tone],
              )}
            >
              <div className="flex flex-col justify-between p-7 md:p-10 lg:w-[42%] lg:shrink-0">
                <div>
                  <p className={cn("t-mono", onAccent ? "text-ink/70" : "text-accent")}>{p.kind}</p>
                  <h3 className="font-display-tight mt-4 text-[clamp(2.5rem,4.5vw,4.5rem)]">
                    {p.name}
                  </h3>
                </div>
                <div className="mt-10">
                  <p className="font-display t-h3 text-balance">{p.title}</p>
                  <p className={cn("t-body mt-5 max-w-[42ch] text-pretty", onAccent ? "text-ink/80" : "text-bone-2")}>
                    {p.summary}
                  </p>
                  <p className={cn("mt-6 border-t pt-4 t-small font-medium", onAccent ? "border-ink/20" : "border-line")}>
                    {p.outcome}
                  </p>
                </div>
              </div>
